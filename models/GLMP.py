...
class GLMP(nn.Module):
    def __init__(self, hidden_size, lang, max_resp_len, path, task, lr, n_layers, dropout):
        #-lr=0.001 -l=1 -hdd=128 -dr=0.2 -dec=GLMP -bsz=8 -ds=babi -t=1
    ...
    def train_batch(self, data, clip, reset=0):
        ...
        max_target_length = max(data['response_lengths']) #所有故事回答的长度最大值
        ...
        # Clip gradient norms
        ec = torch.nn.utils.clip_grad_norm_(self.encoder.parameters(), clip) #梯度裁剪，防止梯度爆炸
    ...
    def encode_and_decode(self, data, max_target_length, use_teacher_forcing, get_decoded_words):
        ...
        if args['unk_mask'] and self.decoder.training:
            ...
            bi_mask = np.random.binomial([np.ones((story_size[0],story_size[1]))], 1-self.dropout)[0]  #使用二巷分布生成mask
            rand_mask[:,:,0] = rand_mask[:,:,0] * bi_mask #生成二项式分布的mask
            ...
            for bi in range(story_size[0]):
                ...
                conv_rand_mask[:end-start,bi,:] = rand_mask[bi,start:end,:] #不知道为什么
            rand_mask = self._cuda(rand_mask)
            conv_rand_mask = self._cuda(conv_rand_mask)
            conv_story = data['conv_arr'] * conv_rand_mask.long() #conv_arr可能没有包含真个故事，context_arr包含了整个故事
