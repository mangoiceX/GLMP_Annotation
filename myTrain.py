...
from utils.utils_Ent_babi import prepare_data_seq  #根据不同的数据源进行调整，得到输入的数据
...
for epoch in range(1): #原来是200
    print("Epoch:{}".format(epoch))  
    # Run the train function
    pbar = tqdm(enumerate(train),total=len(train)) #用来显示进度条
    for i, data in pbar:
        model.train_batch(data, int(args['clip']), reset=(i==0)) #我注释的
